...
# Save Name
SAVE_DIR = 'C:\ExperimentalData\\2025\\2025-05\\2025-05-26_pulsed_esr_GLU_test\\'
SAVE_NAME = 'esr_param_sweep'
# Parameter to update
param = "rf_to_laser_delay"
param_min = 0.0
param_max = 10e-6  # 1 microsecond
# set the number of measurements to take
NUM_MEAS = 10
# Set the total number of sweeps to be taken in each esr
NUM_SWEEPS = 25

param_values = np.linspace(param_min, param_max, NUM_MEAS)  # Example values for the parameter
...
while True:
    try:
        ### update the config with the new parameter 
        setattr(config, param, float(param_values[i]))
...
# Plot the data
plt.figure()
# plot the normalised data without the first point 
plt.imshow(full_data[:,1,1::] / full_data[:,2,1::], 
           aspect='auto', 
           extent=[f_list[1,] - mean_freq, 
                   f_list[-1] - mean_freq, 
                   param_values[0], 
                   param_values[-1]
                   ], 
